...
data_file_train = f'S3R/data/{dataset}/{dataset}.training.csv' 
data_file_test = f'S3R/data/{dataset}/{dataset}.testing.csv' 
ann_file_test = f'S3R/data/{dataset}/{dataset}_ground_truth.testing.json'
if 'universal' in modality:
    univ_dict_file = f'S3R/dictionary/{univ_data}/{univ_data}_dictionaries.{modality}.omp.100iters.npy'
    task_dict_file = None
    tmp_dict_file = f'S3R/dictionary/{dataset}/{dataset}_states.{modality}.npy'
elif 'taskaware' in modality:
    univ_dict_file = None
    task_dict_file = f'S3R/dictionary/{dataset}/{dataset}_dictionaries.taskaware.omp.100iters.50pct.npy'
    tmp_dict_file = None
elif 'univ' in modality and 'task' in modality:
    univ_dict_file = f'S3R/dictionary/{univ_data}/{univ_data}_dictionaries.universal.omp.100iters.npy'
    task_dict_file = f'S3R/dictionary/{dataset}/{dataset}_dictionaries.taskaware.omp.100iters.50pct.npy'
    tmp_dict_file = f'S3R/dictionary/{dataset}/{dataset}_states.universal.npy'
regular_file = f'S3R/dictionary/{dataset}/{dataset}_regular_features-2048dim.training.pickle'
random_state = 49
init_lr = 0.001
