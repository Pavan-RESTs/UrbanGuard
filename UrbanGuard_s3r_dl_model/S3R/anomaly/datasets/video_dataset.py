...
class Dataset(data.Dataset):
    ...
    def _get_video_statistics(self, video_list, memory, regular_file=None, tmp_dict_file=None):
        ...
        def universal_feature(
            ...
            if c != memory.shape[-1]:
                ...
                memory = data_features.reshape(*memory.shape[:-1], -1) 

            x = regular_features.mean(axis=1).reshape(-1, regular_features.shape[-1]) 

            cache = memory.copy() 
            n_slots = cache.shape[0]

            x = normalize(x, norm='l2', axis=1) 
            memory = normalize(memory, norm='l2', axis=1) 
            ...
            topk = np.flip(np.argsort(attn))[:n_slots//2] 

            out = cache[topk] 

            out = np.vstack((out, x.mean(axis=0, keepdims=True))) 
        ...
        if Path(tmp_dict_file).exists():
            ...
            with open(regular_file, 'rb') as fin:
                regular_dict = pickle.load(fin)['feature'] 
    ...
    def __getitem__(self, index):
        ...
        if self.test_mode:
            ...
            video = np.transpose(features, (2, 1, 0)) 
