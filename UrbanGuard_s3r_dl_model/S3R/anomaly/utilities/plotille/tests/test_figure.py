...
def limits(get_limits, set_limits):
    ...
    set_limits(fig)  
    set_limits(fig, min_=1.5)
    assert get_limits(fig) == (1.5, 2.5)  

    set_limits(fig)  
    set_limits(fig, max_=-1.5)
    assert get_limits(fig) == (-2.5, -1.5)  

    fig.clear()
    fig.plot([0], [0])
    set_limits(fig)  
    assert get_limits(fig) == (-0.5, 0.5)  

    fig.clear()
    fig.plot([0.5], [0.5])
    set_limits(fig)  
    assert get_limits(fig) == (pytest.approx(0.45), pytest.approx(0.55))  

    fig.clear()
    fig.plot([0.1, 0.2], [0.1, 0.2])
    set_limits(fig)  
    assert get_limits(fig) == (pytest.approx(0.09), pytest.approx(0.21))  

    set_limits(fig, min_=0)
    assert get_limits(fig) == (0, pytest.approx(0.22))  
    set_limits(fig, max_=2)
    assert get_limits(fig) == (0, 2)  

    set_limits(fig)  
    set_limits(fig, max_=2)
    assert get_limits(fig) == (pytest.approx(-0.09), 2)  

    set_limits(fig)  
    set_limits(fig, max_=0)  
    assert get_limits(fig) == (-1, 0)  

    set_limits(fig)  
    set_limits(fig, min_=1)  
    assert get_limits(fig) == (1, 2)  
