...
from S3R.anomaly.models.modules import deNormal, GlobalStatistics
from S3R.anomaly.models.modules import enNormal
...
class _NonLocalBlockND(nn.Module):
    ...
    def forward(self, x: Tensor, return_nl_map: bool=False):
        ...
        value = self.value(x).view(B, D, -1) 
        value = value.transpose(-2, -1) 

        query = self.query(x).view(B, D, -1) 
        query = query.transpose(-2, -1) 
        key = self.key(x).view(B, D, -1) 
...
class S3R(nn.Module):
    ...
    def __init__(
        ...
        self.k_anomaly: int = quantize_size // 10 
        self.k_regular: int = quantize_size // 10 
    ...
    def forward(
        ...
        video = rearrange(video, 'b n t c -> (b n) t c') 
        ...
        x_video = self.video_embedding(video) 
        x_macro = self.macro_embedding(macro) 
        ...
        video_scores = self.video_classifier(video_embeds) 
        video_scores = video_scores.view(B, N, -1).mean(1) 
        video_scores = video_scores.unsqueeze(dim=2) 

        macro_scores = self.macro_classifier(x_macro.transpose(1, 2)) 
        macro_scores = macro_scores.contiguous().view(-1, N, 1) 
        macro_scores = macro_scores.mean(dim = 1) 
        ...
        anomaly_score = torch.mean(torch.gather(anomaly_scores, 1, idx_abn_score), dim=1)  
        ...
        regular_score = torch.mean(torch.gather(regular_scores, 1, idx_normal_score), dim=1) 
