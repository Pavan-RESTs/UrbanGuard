...
def generate_single_video(video_path, pretrainedpath, frequency, batch_size, sample_mode):
    i3d = i3_res50(400, pretrainedpath)
    i3d.cuda()
    i3d.train(False) 

    videoname = Path(video_path).stem
    temppath = f"temp_{videoname}/"
    Path(temppath).mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        ffmpeg.input(video_path).output(f'{temppath}%d.jpg', start_number=0).global_args('-loglevel', 'quiet').run()
        print("Preprocessing done..")
