...
import requests
from predict_anomaly import main
...
def detect_anomaly(video_path, oversampledCrop):
    ...
    pred = main(input_video_path, pretrained_path, model_path, dictionary_path, oversampledCrop)
...
def upload_file():
    ...
    url = f"http://192.168.184.21:5005/uploads/{filename}" 
...
def analyze_video():
    print("Received a request on /analyze endpoint.")
    ...
    anomaly_detected = detect_anomaly(video_path, request.form['oversampledCrop'])
...
def upload_video_to_localhost(file_path):
    ...
    url = 'http://192.168.184.21:5005/upload' 
    try:
